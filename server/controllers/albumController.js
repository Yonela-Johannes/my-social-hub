...
const getAlbums = async (req, res) => {
  try {
      const album = await Album.find().populate('user').sort({"createdAt": -1}).populate('lovedUsers').populate('songs');
...
const getAlbum = async (req, res) => {
  ...
  try {
      const album = await Album.findById(_id).populate('user').populate('lovedUsers').populate('songs');
      res.status(200).json(album)
  } catch (error) {
    console.log(error.message)
    res.status(500).json({message: error.message})
...
const createAlbum = async (req, res) => {
  const { userId, title, date, cover, artists, songs } = req.body
  ...
  try {
    const newAlbum = new Album({date, cover, artists, songs, title, user: userId, createdAt: new Date().toISOString()})
    await newAlbum.save();
...
module.exports = {createAlbum, getNewAlbums, getAlbums, getAlbum, updateAlbum, deleteAlbum , loveAlbum, getPlaysCount, getDownloadsCount, getAlbumSongs}
