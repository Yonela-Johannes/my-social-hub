...
const TimeSelector = ({ selected, changeTime, type }) => {
  ...
  return (
    <Listbox value={selected} onChange={(e) => {
      ...
      {({ open }) => (
        <>
          <Listbox.Label className='block w-32 text-sm font-medium text-bg_light mb-1'>
            {type === 'openTime' ? 'Opening time' : 'Closing time'}
          </Listbox.Label>
          <div className='relative mt-1'>
            <Listbox.Button className='relative w-full cursor-default rounded-md bg-bg_light py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm'>
